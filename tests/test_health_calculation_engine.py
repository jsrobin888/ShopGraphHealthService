...
from deal_health_service.health_calculation_engine import HealthCalculationEngine
from deal_health_service.ai_processor import AIProcessor
...
class TestHealthCalculationEngine:
    ...
    def test_calculate_health_score_no_events(self, engine, base_promotion):
        ...
        new_score = engine.calculate_health_score([])

        assert new_score == 50  # Should remain at neutral score

    def test_calculate_health_score_single_automated_test_success(
        self, engine, base_promotion, recent_timestamp
    ):
        ...
        new_score = engine.calculate_health_score([event])

        # Should increase health score due to successful test
        assert new_score > 50

    def test_calculate_health_score_single_automated_test_failure(
        self, engine, base_promotion, recent_timestamp
    ):
    ...
    def test_calculate_health_score_community_verification_high_reputation(
        self, engine, base_promotion, recent_timestamp
    ):
    ...
    def test_calculate_health_score_community_verification_low_reputation(
        self, engine, base_promotion, recent_timestamp
    ):
    ...
    def test_calculate_health_score_conflicting_events(
        self, engine, base_promotion, recent_timestamp
    ):
        ...
        new_score = engine.calculate_health_score([automated_event, community_event])
    ...
    def test_calculate_health_score_temporal_decay(self, engine, base_promotion):
        ...
        recent_score = engine.calculate_health_score([recent_event])

        # Calculate with old event only
        old_score = engine.calculate_health_score([old_event])

        # Recent event should have more impact
        assert recent_score > old_score

    def test_calculate_health_score_event_diversity_confidence(
        self, engine, base_promotion, recent_timestamp
    ):
        ...
        single_confidence = engine.get_confidence_score([single_event])

        # Calculate confidence with multiple event types
        diverse_confidence = engine.get_confidence_score(
            [automated_event, community_event]
        )

        # Diverse events should have higher confidence
        assert diverse_confidence > single_confidence

    def test_calculate_health_score_edge_cases(
        self, engine, base_promotion, recent_timestamp
    ):
        ...
        new_score = engine.calculate_health_score([max_reputation_event])
        ...
        new_score = engine.calculate_health_score([min_reputation_event])

        # Should still have some impact but minimal
        assert new_score >= 50

    def test_calculate_health_score_with_existing_signals(
        self, engine, base_promotion, recent_timestamp
    ):
        ...
        base_promotion.raw_verification_signals = [existing_event.model_dump()]
        ...
        new_score = engine.calculate_health_score([new_event])

        # Should consider both existing and new events
        assert new_score < 50  # Should decrease due to failed test
...
class TestAIProcessor:
    ...
    async def test_process_community_tip_positive(self, processor, sample_tip):
        """Test processing a positive community tip."""
        result = await processor.process_community_tip(
            sample_tip.tipText, sample_tip.userReputation or 50
        )
    ...
    async def test_process_community_tip_negative(self, processor):
        ...
        result = await processor.process_community_tip(
            negative_tip.tipText, negative_tip.userReputation or 50
        )
    ...
    async def test_process_community_tip_neutral(self, processor):
        ...
        result = await processor.process_community_tip(
            neutral_tip.tipText, neutral_tip.userReputation or 50
        )
    ...
    async def test_process_community_tip_with_conditions(self, processor):
        ...
        result = await processor.process_community_tip(
            conditional_tip.tipText, conditional_tip.userReputation or 50
        )

        assert "Only works on sale items" in result["conditions"]
        assert "Excludes electronics" in result["exclusions"]
        assert (
            result["structured_data"]["effectiveness"] < 5
        )  # Should be slightly negative due to exclusions

    async def test_process_community_tip_user_reputation_impact(self, processor):
        ...
        high_result = await processor.process_community_tip(
            high_reputation_tip.tipText, high_reputation_tip.userReputation or 50
        )
        low_result = await processor.process_community_tip(
            low_reputation_tip.tipText, low_reputation_tip.userReputation or 50
        )

        # High reputation should have more impact
        assert abs(high_result["health_impact"]) > abs(low_result["health_impact"])

    async def test_process_community_tip_no_user_reputation(self, processor):
        ...
        result = await processor.process_community_tip(
            tip_no_reputation.tipText, tip_no_reputation.userReputation or 50
        )
...
class TestHealthCalculationConfig:
    ...
    def test_config_validation(self):
        ...
        assert abs(config.total_weight - 1.0) < 0.0001
