...
class MessageQueueProcessor:
    ...
    async def _retry_message(self, message: EventMessage, error: Exception):
        ...
        logger.info(f"Retrying message {message.id} in {delay}s (attempt {message.delivery_attempts})")
    ...
    async def _send_to_dlq(self, message: EventMessage, error: Exception):
        ...
        try:
            ...
            logger.warning(f"Message {message.id} sent to DLQ after {message.delivery_attempts} attempts")
            
        except Exception as dlq_error:
            logger.error(f"Failed to send message {message.id} to DLQ: {str(dlq_error)}")
        ...
        mock_events = [
            ...
            },
            {
                ...
                "data": {
                    ...
                    "tipText": "This code works great! Just make sure to spend at least $25.",
                    "timestamp": datetime.utcnow().isoformat(),
                    "userId": "user_456",
                    "userReputation": 75
                }
