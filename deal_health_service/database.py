...
class MockDatabase:
    ...
    async def store_event_result(
        self, result: EventProcessingResult
    ) -> EventProcessingResult:
    ...
    async def get_recent_events(
        self, promotion_id: str, limit: int = 100
    ) -> List[EventProcessingResult]:
        """Get recent events for a promotion."""
        async with self._lock:
            events = [e for e in self.events if e.promotion_id == promotion_id]
    ...
    async def get_promotions_by_merchant(
        ...
        async with self._lock:
            return [p for p in self.promotions.values() if p.merchant_id == merchant_id]
...
class DatabaseService:
    ...
    async def update_promotion_health_score(
        self, promotion_id: str, new_score: int
    ) -> PromotionState:
        """
    ...
    async def store_event_processing_result(
        ...
        result = EventProcessingResult(
            ...
            health_score_before=health_score_before or 50,
