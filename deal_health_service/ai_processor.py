...
class StructuredTipData(BaseModel):
    ...
    effectiveness: int = Field(ge=1, le=10, description="How well the tip suggests the promotion works")
...
class AIProcessor:
    ...
    async def process_community_tip(self, tip_text: str, user_reputation: int = 50) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            health_impact = self._calculate_health_impact(structured_data, user_reputation)
            ...
            logger.info(f"Successfully processed community tip: {health_impact:.3f} impact")
            return result
...
{{
    "conditions": ["List of conditions that must be met (e.g., minimum spend, specific items)"],
    "exclusions": ["List of exclusions or limitations"],
    "effectiveness": <number 1-10 indicating how well the tip suggests the promotion works>,
...
"""
    ...
    async def _call_openai_api(self, prompt: str) -> str:
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are an expert at analyzing promotion tips and extracting structured data."},
    ...
    def _calculate_health_impact(self, structured_data: StructuredTipData, user_reputation: int) -> float:
        """
        ...
        sentiment_multiplier = {
            ...
            "negative": 0.8,
            "neutral": 1.0
        }.get(structured_data.sentiment.lower(), 1.0)
        
        # Calculate final impact
        impact = effectiveness_impact * confidence_multiplier * reputation_multiplier * sentiment_multiplier
        
        # Clamp to valid range
        return max(-1.0, min(1.0, impact))
    
    def _get_fallback_result(self, tip_text: str, user_reputation: int) -> Dict[str, Any]:
        ...
        fallback_data = StructuredTipData(
            ...
            confidence=3,  # Low confidence for fallback
            sentiment=sentiment,
            key_phrases=[]
        )
        ...
        return {
            "structured_data": fallback_data.dict(),
            "health_impact": health_impact,
            "processed_at": datetime.utcnow().isoformat(),
            "user_reputation": user_reputation,
