...
from .message_queue import MessageQueueProcessor, MessageQueueConfig, EventMessage, EventType
from .monitoring import MetricsCollector, trace_function, trace_operation
from .models import (
    AutomatedTestResult,
    CommunityTip,
...
class DealHealthService:
    ...
    def __init__(self, config: Optional[HealthCalculationConfig] = None):
        ...
        self.queue_processor = MessageQueueProcessor(queue_config, self._handle_queue_event)
        
        # Initialize monitoring for observability
        self.metrics = MetricsCollector()
        
        logger.info("DealHealthService - Health Graph System initialized with config: %s", self.config.dict())
    ...
    async def _handle_queue_event(self, message: EventMessage) -> dict:
        ...
        try:
            ...
            result = await self.process_single_event(message.data.get("promotionId"), event)
    ...
    async def process_verification_events(
        ...
        logger.info("Processing %d events for promotion %s in Health Graph System", len(events), promotion_id)
        ...
        try:
            ...
            await self._update_promotion_state(promotion_id, new_score, processed_events)
            ...
            return HealthScoreUpdate(
                ...
                old_score=old_score,
                new_score=new_score,
                change_reason=f"Processed {len(processed_events)} events in Health Graph System",
                events_processed=len(processed_events),
                confidence_score=self.health_engine.get_confidence_score(processed_events)
            )
            
        except Exception as e:
            logger.error("Failed to process verification events in Health Graph System: %s", str(e))
    ...
    async def get_promotions_by_health_range(
        ...
        Returns:
            List of promotion states
        """
        return await self.db_service.get_promotions_by_health_range(min_health, max_health)
    ...
    async def _process_community_tip(self, tip: CommunityTip) -> CommunityTip:
        """
        Process community tip through AI to extract structured data for health calculation.
    ...
    async def _update_promotion_state(
        ...
        """
        Update promotion state with new health score and events in the Health Graph System.
    ...
    async def batch_process_events(
        self, 
        events_by_promotion: dict[str, List[VerificationEvent]]
        ...
        for promotion_id, events in events_by_promotion.items():
            try:
                ...
                logger.error("Failed to process events for promotion %s: %s", promotion_id, str(e))
